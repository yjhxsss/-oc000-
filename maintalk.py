...
def get_typo_dict():
    ...
    if style == "random":
        ...
        return TYPO_STYLES[random.choice(styles)] if styles else DEFAULT_TYPO_DICT
    return TYPO_STYLES.get(style, DEFAULT_TYPO_DICT)
...
# 密码正确时输入框变绿（放在密码处理之后，但要在判断 error 之前）
# 注意：必须在每次 rerun 时都动态判断
...
def render_messages_with_time():
    ...
    for i, msg in enumerate(st.session_state.messages):
        ...
        # 动态已读判断：后面有 assistant 或 tool 消息即为已读
        is_read = False
        if msg["role"] == "user":
...
def inject_auto_timer_js():
    ...
    setTimeout(() => {{
        ...
        if (input) {{
            ...
            nativeInputValueSetter.call(input, 'trig_' + Date.now());
...
if st.session_state.get("auto_trigger") and not st.session_state.auto_timer_trigger_handled:
    trigger_val = st.session_state.auto_trigger
    if trigger_val.startswith("trig_"):
